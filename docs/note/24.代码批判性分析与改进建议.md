...
> - `declareRootProviders` / `useRootService`：全局单例服务。`useRootService` 存在的原因是 `useService` 依赖 Vue 的 provide/inject，只能在组件上下文中调用，而路由中间件、工具类等场景不在组件上下文中，需要一个脱离上下文的获取方式。此组 API 仅适用于 SPA 项目。
...
- 向下查找子容器这个操作在 Angular / inversify 中根本不存在，因为 DI 的基本原则是**父知道子的接口，子不知道父的存在**，反向查找会打破这个边界
