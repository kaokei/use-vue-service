...
>
> 此方案的边界：仅适用于 Nuxt 场景；非 Nuxt 的 Vite SSR、以及 Root 级全局服务在组件外异步初始化的场景，需用户自行处理（如自写 Nuxt 插件）。完整最佳实践见 [27.SSR最佳实践.md](./27.SSR最佳实践.md)。
>
...
| F7 | SSR 客户端数据恢复（Hydration） | 3.6 | 已关闭，决定依赖 Nuxt `useAsyncData`，本库不新增 API，详见 [27.SSR最佳实践.md](./27.SSR最佳实践.md) |
