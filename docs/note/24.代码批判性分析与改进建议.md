...
### 待实现（有明确方案，等待前置依赖或后续迭代）

| # | 事项 | 对应章节 | 前置依赖 |
|---|---|---|---|
| T1 | `@Raw` 扩展支持 class 级别 opt-out | 3.1 | `@kaokei/di` 暴露 `defineMetadata` |
| T2 | `find-service.ts` 改用 `container.getChildren()` | 2.7 | `@kaokei/di` 暴露 `getChildren()` |
| T3 | SSR 客户端数据恢复（Hydration） | 3.6 | 需要独立设计方案 |

### 已修复

| # | 事项 | 对应章节 |
|---|---|---|
| F1 | `useService` / `declareProviders` 错误信息质量 | 1.4 |
| F2 | `@Computed` 中 `getEffectScope(this)` 改为 `getEffectScope(raw)` | 2.1 |
| F3 | `@Raw` field 装饰器删除冗余的 `return ensureRaw` | 2.3 |
| F4 | `declareProviders` / `declareAppProviders` 中删除冗余的 `container = null as any` | 2.6 |
