...
::: tip ✅ 已解决（4.x 版本）
`@Raw` 装饰器已在 4.x 版本中正式实现。基于 TC39 Stage 3 的 field 装饰器和 accessor 装饰器，`@Raw` 可以直接在装饰器层面拦截属性的初始化和赋值，不再需要与 `container.onActivation` 耦合，也不存在下文讨论的"无法在装饰器执行阶段获取实例对象"的问题。
...
详细用法请参考 [API 文档 - Raw](../api/index.md#raw)。
:::
...
> 以下内容基于旧版 TypeScript experimentalDecorators 装饰器规范讨论，在 4.x 版本中已通过 TC39 Stage 3 装饰器彻底解决。
...
> **4.x 版本解决方案**：TC39 Stage 3 装饰器规范中，field 装饰器可以通过返回 `initializer` 函数来拦截字段初始化值，accessor 装饰器可以通过返回 `get`/`set` 函数来拦截属性的读写。这使得 `@Raw` 装饰器可以在不依赖 `container.onActivation` 的情况下独立工作，具备完全的通用性。
