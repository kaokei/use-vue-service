...
`@kaokei/use-vue-service` 是一个轻量级的适用于 Vue 3 项目的状态管理库。当前版本为 **4.0.5**，依赖 `@kaokei/di` 版本 **^5.0.9**。
...
> 以上描述是从组件树角度的粗粒度理解，实际查找基于容器树而非组件树，存在查找起点偏移的细节。准确的查找原理请参考 [API 文档：查找原理](../api/index.md#查找原理)。
